...
const Card = ({ p, onRespond, onRemove }) => {
  ...
  const borderColor = p.status==='ACCEPTED' ? 'border-emerald-500/20' : p.status==='PENDING' ? 'border-amber-500/20' : 'border-dark-500';
  return (
    <div className={`card p-4 ${borderColor}`}>
      <div className="flex items-start gap-3 mb-3">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-display font-bold text-base flex-shrink-0
          ${p.status==='ACCEPTED'?'bg-emerald-500/20 text-emerald-400':p.status==='PENDING'?'bg-amber-500/20 text-amber-400':'bg-dark-600 text-slate-400'}`}>
          {partner.name?.charAt(0).toUpperCase()||'?'}
        </div>
        <div className="min-w-0 flex-1">
          <div className="text-sm font-display font-semibold text-white truncate">{partner.name}</div>
          <div className="text-xs text-slate-500 font-mono truncate">{partner.email}</div>
          {p.updatedAt && p.status==='ACCEPTED' && <div className="text-xs text-slate-600 mt-0.5">Desde {formatDate(p.updatedAt)}</div>}
        </div>
...
export default function PartnershipsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-display font-bold text-white">Vínculos</h1>
        <p className="text-slate-400 text-sm mt-0.5">Conectá con otra persona para compartir finanzas</p>
      </div>

      {/* Invite form */}
      <div className="card p-4 sm:p-5">
        <h2 className="text-sm font-display font-bold text-white mb-3">Invitar por email</h2>
      ...
      {loading ? <div className="text-center py-10 text-slate-500">Cargando...</div> : (
        <>
          {received.length > 0 && (
            <div>
              <h2 className="text-sm font-display font-bold text-white mb-3 flex items-center gap-2">
                ...
                <span className="bg-violet-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">{received.length}</span>
              </h2>
          ...
          {pending.filter(p=>p.isSender).length > 0 && (
            <div>
              <h2 className="text-sm font-display font-bold text-white mb-3">⏳ Pendientes enviadas</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          {active.length > 0 && (
            <div>
              <h2 className="text-sm font-display font-bold text-white mb-3">✓ Vínculos activos</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          {rejected.length > 0 && (
            <div>
              <h2 className="text-sm font-display font-bold text-white mb-3 opacity-50">Historial</h2>
          ...
          {partnerships.length === 0 && (
            <div className="card p-10 text-center">
              ...
              <div className="text-white font-display font-bold mb-1">Sin vínculos</div>
              <div className="text-slate-400 text-sm">Invitá a alguien con su email</div>
            </div>
          )}
        </>
      )}
